...
`notes/knowledge/design/stitch_personal_universe_3d_v2/personal_universe_3d/DESIGN.md`:
...
- Add the test step to CI (`.github/workflows/ci.yml` has a reserved TODO) and
  update `notes/knowledge/frontend/source-overview.md`.
...
- Focus trap in GeneratingOverlay; `prefers-reduced-motion` disables the
  spinning animation.
- Sweep remaining magic numbers into named constants (notes/rules/coding-style.md).
- Delete dead code/imports; re-sync `notes/knowledge/frontend/source-overview.md` with the new
  structure.
