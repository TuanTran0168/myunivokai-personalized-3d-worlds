...
- No new NATS user needed for Synadia production — one shared `nats.creds`
  ...
  `notes/skills/auth-analytics-first-deploy-checklist.md` for why). A local-only
...
- `notes/knowledge/backend/source-overview.md` gets one paragraph: the backend is Go except
