...
— xem `notes/plans/services/auth-and-admin-plan.md#tokens`. Sinh một seed 32-byte,
...
> `notes/plans/services/auth-and-admin-plan.md#passwords` (không có mật khẩu mặc định
...
- Đó chính là lý do cron/keep-alive định kỳ bị loại, và tại sao cơ chế đánh thức theo nhu cầu là phương án duy nhất vừa ngân sách: nó gọi đúng một lần cho mỗi service đang ngủ trong mỗi cửa sổ khoá, do request thật kích hoạt, rồi để service ngủ lại. Xem `notes/plans/architecture/service-wake-mechanism.md`.
...
- **Hệ quả "ngủ đông" nói trên nay đã được xử lý ở tầng gateway**, không phải bằng cách giữ service thức.
  ...
  - Chi tiết: `notes/plans/architecture/service-wake-mechanism.md`.
...
- **`auth-service` ngủ đông sau ~15 phút không có traffic**, giống hệt
  ...
  `notes/plans/architecture/service-wake-mechanism.md`. Không phải lỗi cấu hình deploy.
