...
/**
 * Every document is rendered per request, and this is a requirement of the
 * Content-Security-Policy rather than a data-freshness preference.
 *
 * `middleware.ts` sends `script-src 'self' 'nonce-…' 'strict-dynamic'`, and a
 * nonce can only match if the HTML carrying it was produced by the same
 * request that produced the header. A PRERENDERED page's HTML is written at
 * build time, so its script tags have no nonce — and because `'strict-dynamic'`
 * disables host-source allowlists, the `'self'` beside it stops applying too.
 * The browser then refuses every application chunk. Measured, not reasoned
 * about: on `/sign-in` that was 12 refusals, 20 script tags with 0 nonces, an
 * empty React root, and **no page error at all** — which is why `tsc`,
 * `next lint`, `next build` and 801 unit tests were all green over an app that
 * did not become interactive in production. See `S3-CSP-001`.
 *
 * The two alternatives were rejected, and the reasons belong here because this
 * line looks like a performance regression to anyone who finds it later:
 *
 * - **Hash the inline scripts** instead of nonce-ing them. Next emits 7 inline
 *   `<script>` tags here, including the `self.__next_f` flight payload, whose
 *   contents change with the route and the build. A hash list would have to be
 *   regenerated by the build that invalidates it.
 * - **Add `'unsafe-inline'` as a fallback.** It is not a fallback: a browser
 *   that understands nonces ignores `'unsafe-inline'` entirely, so it would
 *   apply on exactly the prerendered pages that lack a nonce — granting inline
 *   script precisely where the policy is the session's only defence.
 *   `lib/contentSecurityPolicy.ts` holds the full argument.
 *
 * Declared explicitly rather than by calling `headers()` somewhere in the tree
 * to force dynamic rendering as a side effect. That works, and it is how Next's
 * own nonce examples read, but it makes an unused-looking call load-bearing —
 * and the next reader to tidy it away would restore a silent total outage.
 */
