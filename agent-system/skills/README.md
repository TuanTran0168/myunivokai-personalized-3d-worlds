...
Two of these are **invokable by name**. `/deploy` routes to the right runbook
below rather than letting one be picked by its title, and `/record-to-memory`
writes an execution record into [../memory/](../memory/README.md). Their entry
points live in `.claude/skills/`; the substance stays here.
...
A skill that should be invokable by name needs a matching
`.claude/skills/<name>/SKILL.md` with `name` and `description` frontmatter. The
`description` is what decides whether it is reached, so it should say when to
use it, not what it is.
