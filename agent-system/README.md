...
The project's knowledge base, and the wiring that makes it operate. It is two
things at once on purpose: every file here is written to be read by a person,
and several of them are also loaded and executed by Claude Code without anyone
opening them.

It used to be called `notes/` and be sorted by discipline — `fe/`, `be/`,
`ops/`, `vision/`. That name and that sorting made the same claim: this is where
documents go to sit. The folders below are sorted by the question you actually
have, and which question you have depends on where in a piece of work you are:
...
Claude Code discovers agents and skills only under `.claude/`, so that directory
holds thin entry points and this one holds the substance. There is exactly one
copy of every definition, and it is here.

| Wiring | Loads | Effect |
| --- | --- | --- |
| [`CLAUDE.md`](../CLAUDE.md) | `rules/` | Every session starts having read the three gates. Nobody has to remember to. |
| `.claude/agents/*.md` | `agents/*.md` | `frontend`, `backend`, `operations`, `research` are dispatchable subagents. Each one's first instruction is to read its definition here. |
| `.claude/skills/*/SKILL.md` | `skills/*.md`, `memory/` | `/deploy` and `/record-to-memory` are invokable by name. |
| [`project-context.json`](project-context.json) | — | The map in a form a program can parse: areas and their authority, service languages and ports, the world families and what adding one really costs. |

Changing a definition here changes what runs. Changing a file under `.claude/`
...
Everything below moved out of `notes/`, and every move is recorded in git as a
rename — so `git log --follow` still reaches the full history of each file, and
`git log --follow -- agent-system/knowledge/frontend/source-overview.md` reads
back past two renames without a gap.

| Old path (under `notes/`) | New path (under `agent-system/`) |
| --- | --- |
| `coding/` | [rules/](rules/) |
