...
| [end-user-identity-and-ownership.md](end-user-identity-and-ownership.md) | **Proposed 2026-09-02, not approved, no code.** Graduates Track A into a contract and answers `DEFERRED-AUTH-001`'s seven questions. Read §2 first: two thirds of the identity layer is already shipped, because Sprint 4 built staff auth as one half of a two-audience design — `accounts.kind` already admits `end_user`, roles and permissions already carry an audience, and a `web` token is already rejected at the admin edge by a test. It corrects Track A in four places found in the NATS ACLs and `dna-service`'s schema, the largest being that **`library-service` is not needed** — `generation_jobs` already joins profile to world to family for all three families. **If approved it overrides principle 9 and D19 below.** |
