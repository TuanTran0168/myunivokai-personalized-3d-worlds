...
| [end-user-identity-and-ownership.md](end-user-identity-and-ownership.md) | **Approved and BUILT. Corrected 2026-09-05: this row said "Proposed 2026-09-02, no code yet" while all 21 stories of [Sprint 08](../sprints/sprint-08-2026-09-02/README.md) are `Implemented` — Phases A, B and C are in production, so principle 9 and D19 below are now overridden in fact and not merely conditionally.** Phase D (email, then OAuth) is deliberately last and unstarted; Phase E is answered by the row below. Twenty decisions were taken the same day across eight rounds (§16) and nothing is left open**, several of which cut scope — no email, no OAuth, no passkeys and no account-deletion feature in the first release, which also retires this plan's own §3.4 correction. The sprint covers Phases A-C only. Graduates Track A into a contract and answers `DEFERRED-AUTH-001`'s seven questions. The decision most likely to surprise a reader who knows the admin app: the **product session is a bearer token, not a cookie**, because web-on-Vercel and gateway-on-Render are two different sites and a session cookie there is a third-party cookie that fails silently on iOS. Read §2 first: two thirds of the identity layer is already shipped, because Sprint 4 built staff auth as one half of a two-audience design — `accounts.kind` already admits `end_user`, roles and permissions already carry an audience, and a `web` token is already rejected at the admin edge by a test. It corrects Track A in four places found in the NATS ACLs and `dna-service`'s schema, the largest being that **`library-service` is not needed** — `generation_jobs` already joins profile to world to family for all three families. **If approved it overrides principle 9 and D19 below.** |
| [phase-e-what-ownership-unlocks.md](phase-e-what-ownership-unlocks.md) | **Written 2026-09-04 as a proposal; brought onto `refactor/be/family-service-shared-platform` on 2026-09-05 with a correction note, not a merge of its own branch's stale code changes.** Fills in the Phase E `end-user-identity-and-ownership.md` §13 left empty, by first splitting "ownership means livelier statistics" into the staff half §15 forbids and the owner-facing half that is a genuine product feature — the same distinction the row below independently arrives at for the admin app. **Its own §7 found the share-URL defect that the row below fixed; read the correction note near the top before §7, which now describes two fixed defects as open ones.** Two more claims (telemetry's on/off flag, a config file path) are also stale and are flagged rather than edited. §3-6's fork, triage and sequencing were not touched by later work and stand as written |
| [admin-surface-and-family-service-duplication.md](admin-surface-and-family-service-duplication.md) | **Half executed on 2026-09-05 — read its §14 first.** The share-URL fix and the Tier 0 extraction into [`shared/family-platform/go`](../../../shared/family-platform/go/README.md) are built; the admin items are not. §14 records where the plan was wrong, including a Tier 0 claim that would have sent the next reader down a path that does not exist. Answers "what is left on the admin side, or should the backend be refactored" by showing the two are one decision: every admin feature that reaches a family service is written three times today. **Read its §4 before anything else** — surveying for it found that every published world in every family returns a 404 share URL, and that ocean has no `PUBLIC_WEB_URL` in production at all, so its links point at `localhost`. Measures the family-service duplication rather than asserting it (`world_ownership.go` is byte-identical across three services; 18 of the 24 commits that ever touched these files had to edit two or more services at once) and tiers the extraction by what each file **imports**, so the 873 lines that need no abstraction move separately from the ~1,300 that need a design decision. Also records the one admin feature that must never be built: §15 forbids `owner_account_id` in `myunivokai_analytics`, so staff cannot be shown who owns a world |
