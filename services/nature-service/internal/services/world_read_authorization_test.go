...
// newReadAuthorizationFixture exists because universe's newTestWorldService
// takes a store while this one builds its own. Everything below this line is
// identical across the three families, so the difference is isolated here
// rather than spread through the tests.
func newReadAuthorizationFixture(t *testing.T) (*WorldService, *repositories.MemoryStore) {
	t.Helper()
	return newTestWorldService(t)
}
