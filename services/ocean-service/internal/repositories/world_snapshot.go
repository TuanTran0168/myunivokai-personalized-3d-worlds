...
// universe-service's and nature-service's copies of it.
const snapshotFamily = contracts.WorldFamilyOcean
...
func newWorldSnapshot(world models.World, variantCount, selectedVariantNo int, selectedVariantSeed string, publishedAt *time.Time) contracts.WorldSnapshot {
	...
	return contracts.WorldSnapshot{
		...
		TraitScores: contracts.TraitScores{
			Creativity: world.OceanDNA.TraitScores.Creativity,
			Discipline: world.OceanDNA.TraitScores.Discipline,
			Curiosity:  world.OceanDNA.TraitScores.Curiosity,
			Energy:     world.OceanDNA.TraitScores.Energy,
			Focus:      world.OceanDNA.TraitScores.Focus,
		},
...
// guard against the failure this family's plan calls the easiest to forget,
// BECAUSE NOTHING FAILS WHEN YOU DO: a mutation that skips it simply stops
// feeding the read model, and ocean worlds quietly never appear in the admin
// app. world_snapshot_test.go is what catches the omission.
