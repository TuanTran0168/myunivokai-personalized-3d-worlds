...
func TestEveryMutationEmitsAWorldChangeEvent(t *testing.T) {
	...
	for _, mutation := range mutations {
		t.Run(mutation.name, func(t *testing.T) {
			...
			message := messages[0]
			if message.Subject != contracts.OceanWorldChangedEventSubject {
				t.Fatalf("%s published on %q, want %q", mutation.name, message.Subject, contracts.OceanWorldChangedEventSubject)
			}
			...
			if snapshot.Family != contracts.WorldFamilyOcean || snapshot.WorldID != worldID {
...
// every observed rarity rate describe a sea nobody is looking at.
...
func TestCreateWorldCarriesTheFirstSnapshotOnTheCompletedEvent(t *testing.T) {
	...
	if messages[0].Subject != contracts.OceanCompletedEventSubject {
		t.Fatalf("CreateWorld published on %q, want %q", messages[0].Subject, contracts.OceanCompletedEventSubject)
	}
