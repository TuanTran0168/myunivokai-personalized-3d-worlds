...
func newTestWorldService(t *testing.T) (*WorldService, *repositories.MemoryStore) {
	...
	serviceConfig := config.Config{PublicWebURL: "http://localhost:41300/ocean", ShareSlugLength: 10}
	return NewWorldService(serviceConfig, store, NewOceanConfigBuilder()), store
}

func validComposeEnvelope() contracts.Envelope[contracts.ComposeWorldData] {
	return contracts.NewEnvelope("01K0OCEANF1234567890", contracts.ComposeWorldData{
		Family:       contracts.WorldFamilyOcean,
...
func TestComposeWorldStoresOceanVariant(t *testing.T) {
	...
	if !strings.HasPrefix(response.Variant.Seed, "OCN-") {
		t.Fatalf("world seed %q must carry the OCN- prefix", response.Variant.Seed)
	}
	if response.Variant.Config.SceneType != oceanSceneType || response.Variant.Config.SchemaVersion != oceanSchemaVersion {
		t.Fatalf("expected ocean %s config, got %s/%s", oceanSchemaVersion, response.Variant.Config.SceneType, response.Variant.Config.SchemaVersion)
	}
	if len(response.Variant.Config.Landmarks) != len(response.OceanDNA.Landmarks) || response.OceanDNA.Landmarks[0].Type != "Interest Landmark" {
		t.Fatalf("unexpected ocean DNA mapping: %#v", response.OceanDNA.Landmarks)
	}
}
...
func TestRegenerateVariantIsDeterministicFromStoredDNA(t *testing.T) {
	...
	rebuilt := NewOceanConfigBuilder().Build(BuildOceanConfigInput{
		DNA:       created.OceanDNA,
...
func TestSelectPublishAndGetPublicWorld(t *testing.T) {
	...
	if err != nil || publicWorld.Variant.Config.SceneType != oceanSceneType {
		t.Fatalf("get public ocean: %#v, %v", publicWorld, err)
	}
}

// Twin of the universe-service test: the gateway's public share cache is keyed by
// slug, so a variant mutation has to hand the slug back or the ocean share page
