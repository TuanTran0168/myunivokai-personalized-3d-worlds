...
const (
	...
	failedEventMessageStage  = ":ocean-failed"
	compositionFailedCode    = "OCEAN_COMPOSITION_FAILED"
	compositionFailedMessage = "The ocean world could not be composed. Please try again."
)
...
// NATSHandler owns the Ocean service's transport-specific request handling.
...
func (handler *NATSHandler) HandleComposition(ctx context.Context, message *nats.Msg) error {
	...
	if err := decodeEnvelope(message.Data, &envelope); err != nil {
		return fmt.Errorf("decode ocean command: %w", err)
	}
...
func (handler *NATSHandler) PublishCompositionFailure(ctx context.Context, message *nats.Msg) error {
	...
	failedEnvelope := contracts.NewEnvelope(composeEnvelope.JobID, contracts.FamilyFailedData{
		Family: contracts.WorldFamilyOcean, ProfileID: composeEnvelope.Data.ProfileID, DNAVersionID: composeEnvelope.Data.DNAVersionID,
		Code: compositionFailedCode, Message: compositionFailedMessage,
	})
	payload, err := json.Marshal(failedEnvelope)
	if err != nil {
		return fmt.Errorf("marshal ocean failed event: %w", err)
	}
	failedMessage := nats.NewMsg(contracts.OceanFailedEventSubject)
	...
	if _, err := handler.eventPublisher.PublishMsg(failedMessage, nats.Context(ctx)); err != nil {
		return fmt.Errorf("publish ocean failed event: %w", err)
	}
	return nil
}
...
func (handler *NATSHandler) respondWithResult(message *nats.Msg, jobID string, successStatus int, payload any, err error) {
	...
	if err != nil {
		log.Error().Err(err).Str("request_id", jobID).Msg("ocean query failed")
...
func (handler *NATSHandler) respond(message *nats.Msg, response any) {
	...
	if err != nil {
		log.Error().Err(err).Msg("marshal ocean NATS response")
		return
	}
	if err := handler.responsePublisher.Publish(message.Reply, payload); err != nil {
		log.Error().Err(err).Msg("publish ocean NATS response")
	}
}
