...
func (runtime *Runtime) Run(ctx context.Context) error {
	...
	queryBindings := []queryBinding{
		...
		{subject: contracts.UniverseWorldUnpublishSubject, handler: runtime.natsHandler.HandleWorldUnpublishQuery},
	...
	for {
		...
		for _, message := range messages {
			...
			if err := runtime.natsHandler.HandleComposition(ctx, message); err != nil {
				...
				log.Warn().Str("subject", message.Subject).Dur("duration", time.Since(messageStartTime)).Err(err).Msg("universe message processing failed, will retry")
...
// might carry a profile or world's contents.
func (runtime *Runtime) loggedQuery(handler nats.MsgHandler) nats.MsgHandler {
	return func(message *nats.Msg) {
		queryStartTime := time.Now()
		handler(message)
		log.Info().Str("subject", message.Subject).Dur("duration", time.Since(queryStartTime)).Msg("universe query answered")
	}
}
